...
export default function JsonFormatter() {
  ...
  return (
    <div className="flex-1 text-ink flex flex-col overflow-hidden">
      {/* 顶部工具栏 - 紧凑设计（玻璃面板范式：贴边去左右/顶部描边，底部描边由玻璃边框承担） */}
      <div className="glass-panel border-x-0 border-t-0 rounded-none px-4 py-2 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            {/* tint chip：低饱和色底 + 同色描边 + 同色图标（替换实心 bg-success 色块） */}
            <div className="w-10 h-10 tint tint-emerald rounded-xl">
              <Code className="w-4 h-4" />
            </div>
            <h1 className="text-lg font-bold">
              {/* gradient-text 会置 color: transparent，故只包文字词组 */}
              <span className="gradient-text">JSON</span>格式化
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* 压缩按钮：保留语义警示黄，收敛为 Badge warning 同款低饱和规格（secondary 打底避免与品牌渐变主按钮冲突） */}
          <Button
            onClick={minifyJson}
            variant="secondary"
            size="sm"
            className="bg-[rgba(251,191,36,0.12)] text-accent-warning border border-[rgba(251,191,36,0.3)]"
          >
