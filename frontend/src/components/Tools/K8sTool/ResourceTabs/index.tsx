...
export const ResourceTabs: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden min-h-0">
      <Tabs value={resourceType} onValueChange={(v) => setResourceType(v as 'pods' | 'workloads' | 'nodes' | 'events')} className="flex-1 flex flex-col overflow-hidden min-h-0">
        <TabsList className="mx-4 bg-transparent h-auto p-0 justify-start border-b border-border bg-surface-1/30 shrink-0">
          {TABS.map((tab) => (
