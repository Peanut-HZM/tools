  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <p className="text-slate-400 text-sm">管理大模型供应商（API Key、连接地址等）</p>
        <button
          onClick={handleAdd}
      ...
      {loading ? (
        ...
        <div className="bg-slate-700 rounded-lg p-12 text-center border border-slate-600">
          ...
          <h3 className="text-lg font-medium text-white mb-2">暂无供应商</h3>
          <p className="text-slate-400 mb-4">新建供应商以开始配置大模型</p>
          ...
          <table className="w-full">
            <thead className="bg-slate-800">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm font-medium text-slate-300">厂商</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-slate-300">Base URL</th>
            ...
            <tbody className="divide-y divide-slate-600">
              {providers.map((p) => (
                <tr key={p.id} className="hover:bg-slate-600/50 transition-colors">
                  <td className="px-4 py-3">
                    <div className="text-white font-medium">{p.name}</div>
                    {p.notes && (
                      <div className="text-xs text-slate-500 mt-1 truncate max-w-[200px]" title={p.notes}>
                        📝 {p.notes}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="text-slate-300 text-sm truncate max-w-[250px]" title={p.base_url}>
                      {p.base_url}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      <button
                        ...
                        className="px-2 py-1 text-xs bg-blue-600/20 text-blue-400 border border-blue-500/30 rounded hover:bg-blue-600/30 transition-colors disabled:opacity-50"
                      >
