...
# SSRF 防护：黑名单与 DNS 检查统一收敛到 image_provider/base.py
...
def _is_safe_http_url(raw: Any) -> bool:
    ...
    - 拒绝解析到内网/loopback/link-local IP 的 hostname（含 IPv6 AAAA 记录）
    - 限制长度
    ...
    # 4. DNS 解析 + IP 黑名单检查（检查所有 A/AAAA 记录，防止 DNS rebinding / IPv6 绕过）
