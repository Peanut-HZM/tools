...
def generate_uuid():
    ...
    new_uuid = uuid.uuid4()
    return {"uuid": str(new_uuid), "uuid_hex": new_uuid.hex}
...
def generate_api_key(length: int):
    ...
    return {"api_key": key[:length], "api_key_hex": secrets.token_hex(length // 2)}


def generate_base64_string(length: int):
    """生成Base64编码的随机字符串"""
    # 由于Base64编码会增加约33%的长度，我们需要生成更少的字节
    ...
    # 截断到指定长度（去除可能的填充字符）
    return {"base64_string": base64_string[:length]}
...
async def generate_key(request: KeyGenerateRequest):
    ...
    try:
        if algorithm == "rsa":
            ...
            result = generate_base64_string(key_size)
        else:
