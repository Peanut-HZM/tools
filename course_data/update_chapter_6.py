...
# 新的第 6 章内容
CHAPTER_6_CONTENT = """## 全景视角：三大 Spec 驱动工具深度对比
        ...
        "questions": [
            {
                ...
                "options": [
                    {
                        "option_text": "OpenSpec - 平衡灵活性和结构性，支持多工具",
                        "option_index": 0,
                    },
                    {
                        "option_text": "Spec-Kit - GitHub 官方，企业级支持",
                        "option_index": 1,
                    },
                    {
                        "option_text": "Superpowers - 高度自动化，零配置",
                        "option_index": 2,
                    },
            ...
            },
            {
                ...
                "options": [
                    {
                        "option_text": "直接编写代码，然后补充规格文档",
                        "option_index": 0,
                    },
                    {
                        "option_text": "/speckit.specify → /speckit.plan → /speckit.tasks → 实现",
                        "option_index": 1,
                    },
                    {
                        "option_text": "先写测试，再写实现代码，最后写规格",
                        "option_index": 2,
                    },
                    {
                        "option_text": "根据上下文自动触发，无需手动调用命令",
                        "option_index": 3,
                    },
                ],
            },
            {
                "question_text": "【决策】企业级项目（20+ 人团队）需要严格的合规审查和统一规范，应该选择哪个工具？",
                "question_type": "single",
                "correct_answer": "2",
                "explanation": "Spec-Kit 专为大型企业设计，提供严格流程和统一规范。宪法驱动和模板约束确保团队一致性。",
                "order": 2,
                "options": [
                    {"option_text": "Superpowers - 自动化程度最高", "option_index": 0},
                    {"option_text": "OpenSpec - 灵活性最好", "option_index": 1},
                    {
                        "option_text": "Spec-Kit - 企业级流程和严格审查",
                        "option_index": 2,
                    },
            ...
            {
                "question_text": "【场景】在 Cursor 中开发个人项目，希望零配置快速启动，应该选择哪个工具？",
                "question_type": "single",
                "correct_answer": "1",
                "explanation": "Superpowers 是 Cursor 原生插件，通过/add-plugin superpowers 一键安装，零配置，技能自动触发，最适合个人快速开发。",
                "order": 3,
                "options": [
                    {
                        "option_text": "OpenSpec - 需要 npm 安装和初始化",
                        "option_index": 0,
                    },
                    {
                        "option_text": "Superpowers - Cursor 插件市场一键安装",
                        "option_index": 1,
                    },
                    {
                        "option_text": "Spec-Kit - 需要 uv 安装和宪法配置",
                        "option_index": 2,
                    },
                    {"option_text": "三个工具都不适合", "option_index": 3},
                ],
            },
            {
                "question_text": "【多选】以下哪些是 Superpowers 的核心技能？（多选）",
                "question_type": "multiple",
                "correct_answer": "0,1,2,3",
                "explanation": "brainstorming、writing-plans、test-driven-development、systematic-debugging 都是 Superpowers 的核心技能。所有选项都正确。",
                "order": 4,
                "options": [
                    {
                        "option_text": "brainstorming - 需求探索和设计细化",
                        "option_index": 0,
                    },
                    {"option_text": "writing-plans - 详细实现计划", "option_index": 1},
                    {
                        "option_text": "test-driven-development - RED-GREEN-REFACTOR 循环",
                        "option_index": 2,
                    },
                    {
                        "option_text": "systematic-debugging - 4 阶段根本原因分析",
                        "option_index": 3,
                    },
                ],
            },
...
```""",
        "extra_data": {"cheatsheet_type": "commands"},
    },
    {
        "resource_type": "reference",
        "title": "三大工具官方资源链接",
        "content": """## OpenSpec 官方资源
...
- **技能库**: ~/.cursor/skills/superpowers/""",
        "extra_data": {"resource_type": "links"},
    },
]


def update_chapter_6():
    # 读取原始 JSON 文件
    json_path = Path(
        "/Users/huazhongmin/IdeaProjects/tools/course_data/course-export-2026-03-12/course-export.json"
    )
    ...
    # 查找第 6 章（order: 6）
    chapter_6_index = None
    for i, chapter in enumerate(data["chapters"]):
        if chapter["order"] == 6:
            chapter_6_index = i
            break

    if chapter_6_index is None:
        print("错误：未找到第 6 章")
        return False

    # 更新第 6 章内容
    data["chapters"][chapter_6_index]["content"] = CHAPTER_6_CONTENT
    data["chapters"][chapter_6_index]["quizzes"] = CHAPTER_6_QUIZZES
    data["chapters"][chapter_6_index]["resources"] = CHAPTER_6_RESOURCES

    # 更新统计信息
    total_quizzes = sum(
        len(chapter.get("quizzes", [{}])[0].get("questions", []))
        if chapter.get("quizzes")
        else 0
        for chapter in data["chapters"]
    )
    total_questions = sum(
        sum(len(quiz.get("questions", [])) for quiz in chapter.get("quizzes", []))
        for chapter in data["chapters"]
    )
    total_options = sum(
        sum(
    ...
    data["export_stats"]["quizzes_count"] = sum(
        len(chapter.get("quizzes", [])) for chapter in data["chapters"]
    )
    data["export_stats"]["questions_count"] = total_questions
    data["export_stats"]["options_count"] = total_options

    # 保存更新后的 JSON 文件
    output_path = Path(
        "/Users/huazhongmin/IdeaProjects/tools/course_data/course-export-2026-03-12/course-export-updated.json"
    )
