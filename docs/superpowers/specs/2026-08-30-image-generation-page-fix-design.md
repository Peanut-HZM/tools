...
## 10. v4 迭代：意图推断 + 提示词工程（已实现并验收）

用户需求：根据用户输入推断意图、丰富提示词，让生图模型更准确地出图（通过优化 agent 流程）。

实现（三层）：
1. **Agent 层**（image_gen_agent system_prompt）：调用 image_gen 前必须完成
   "意图推断 → 提示词扩写"——从用途推断构图预期（封面留白/头像特写/配图氛围），
   按主体细节→环境→构图视角→光线氛围→风格质感→色调情绪扩写；
   用户明确的风格/比例/数量为硬约束原样保留（16:9→1792x1024 等映射）。
2. **工具层**（prompt_refiner v4）：专业文生图 prompt 工程模板
   （意图推断 + 七要素结构化 + 硬约束保留规则）；移除"英文输入跳过"短路
   （英文简短描述同样丰富）；max_tokens 300→500。
3. **Runtime 层**：编造链接清洗前移到 text_delta 发射前（无工具调用轮次中
   出现的 image-gen URL 一律替换为无效警告，用户不再点到大模型幻觉链接）；
   每轮重置工具产出集合。

前端：气泡内新增可折叠"✨ 优化后的生图提示词"（展示 revised_prompt，
用户可直观看到丰富效果）。

验收（2026-08-30）：
- 单测：prompt_refiner 31 passed（含英文输入也丰富的行为变更）、
  runtime 清洗单测、幻觉防护端到端测试
- 真实生成：输入 "a sunny beach with palm trees, warm and cozy, 16:9, 1 image"
  → revised_prompt 扩写为七要素结构化 prompt（主体细节/环境/光线/氛围/
  风格/构图/色调），硬约束 16:9 保留，图片落 OSS
- 回归：pytest tests/harness 768 passed；前端 build + tsc 通过
